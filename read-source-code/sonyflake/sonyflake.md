...
redis 的skiplist用的double表示score
double 类型在大多数现代编程语言中遵循 IEEE 754 标准，通常是 64 位双精度浮点数。这种类型的浮点数可以表示非常大和非常小的数值，但它并不是精确表示所有整数的理想选择。

对于 64 位双精度浮点数（double），它可以精确表示的整数范围是从 -2^53 + 1 到 2^53 - 1。这是因为 double 类型有 53 位有效数字（包括隐含的 1 位），这意味着它可以精确表示 53 位以内的整数。

具体来说：

最小值：-9007199254740991（即 -2^53 + 1）

最大值：9007199254740991（即 2^53 - 1）
